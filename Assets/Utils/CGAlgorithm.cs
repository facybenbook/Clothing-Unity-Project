...
public class CGAlgorithm
{
    ...
    public static Vector3 NormalOf3Points(Vector3 a, Vector3 b, Vector3 c)
    {
        var s0 = b - a;
        var s1 = c - a;

        var perp = Vector3.Cross(s0, s1);
        var length = perp.magnitude;
    ...
    public static Vector3 NegativeNormalOfPoints(Vector3 a, Vector3 b, Vector3 c )
    {
        var s0 = b - a;
        var s1 = c - a;

        var cross = Vector3.Cross(s0, s1).normalized;//.Int();
        cross = new Vector3 ( ( int ) cross.x, ( int ) cross.y, ( int ) cross.z );
    ...
    * */
    ...
    public static float Area(Vector2 u, Vector2 v )
    {
        return Perp ( u, v ) / 2.0f;
    }

    public static float Area(Vector2 p0, Vector2 p1, Vector2 p2 )
    {
        return Perp (p0, p1, p2 ) / 2.0f;
    }
    ...
    public static float PointToSegementDistance ( Vector2 p, Vector2 e0, Vector2 e1, out Vector2 position)
    {
        position = p;
        Vector2 u = p - e0;
        Vector2 v = e1 - e0;
    ...
    public static bool IsOn(Vector2 p0, Vector2 p1, Vector2 p2 )
    {
        return ( Mathf.Abs ( Perp ( p1, p2, p0 ) ) < SMALL_NUM )
            && ( Dot ( p0 , p1, p2 ) <= 0 );
    }
